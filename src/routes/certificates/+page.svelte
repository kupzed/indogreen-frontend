        ...
        {#each items as item (item.id)}
          <li>
            ...
            <div class="flex justify-end px-4 py-2 sm:px-6 space-x-2">
              <button on:click|stopPropagation={() => openDetailDrawer(item)} class="inline-flex items-center px-3 py-1.5 border border-transparent rounded-md shadow-sm text-xs font-medium text-indigo-700 bg-indigo-100 hover:bg-indigo-200">
                Detail
              </button>
            ...
            {#each items as item (item.id)}
              <tr>
                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-900">
                  <a href={`/certificates/${item.id}`} class="text-indigo-600 hover:text-indigo-900">{item.name}</a><br>
                  <span class="text-xs text-gray-500">{item.barang_certificate?.name || '-'}</span>
                </td>
                ...
                <td class="whitespace-nowrap px-3 py-4 text-sm text-indigo-600">{#if item.attachment}<a href={item.attachment} target="_blank" rel="noreferrer" class="hover:underline">Lihat</a>{:else}-{/if}</td>
